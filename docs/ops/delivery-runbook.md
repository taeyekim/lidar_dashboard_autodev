...
`LIVE_TCP_READY`, PRESENT manual evidence, non-blocking strict security
evidence, empty `Residual Field Gates`, and fresh delivery/readiness/security/index/
closure references.
...
- `artifacts/field-closure-plan/<timestamp>/manifest.json` and `manifest.md` from `npm run field:closure-plan`; confirm `Field Rehearsal Follow-up Actions` lists owner, target recheck date, next action, and done-when criteria.
...
- Confirm the handover package `Git commit`, `Git branch`, and `Working tree clean` fields match the dev revision being delivered.
- `npm.cmd run verify:final-status` or `npm run verify:final-status` result; confirm READY/COMPLETE claims use fresh referenced artifacts, including security evidence, and no open field/security/manual gates.
