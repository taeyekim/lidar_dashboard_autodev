...
npm.cmd run local:verification -- --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"
...
Use `npm.cmd run local:verification -- --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` after local contract checks to write
`artifacts/local-verification/<timestamp>/manifest.json` plus `manifest.md`.
This evidence records `npm.cmd run smoke` logs, pass/fail counts, Git commit,
push state, and working tree cleanliness. It is useful for handover review but
does not replace field runtime, hardware TCP ACK, external CI, or reviewer-signed
evidence.
...
- `artifacts/local-verification/<timestamp>/manifest.json` and `manifest.md` from `npm run local:verification`; confirm `npm.cmd run smoke` passed on the delivery source revision and remember this local evidence does not replace field runtime, hardware TCP ACK, external CI, or reviewer-signed evidence.
