...
- [ ] `npm run local:verification -- --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/local-verification/<timestamp>/manifest.json` plus `manifest.md`, records `npm.cmd run smoke` logs, pass/fail counts, Git commit, push state, and working tree cleanliness, and remains local contract evidence rather than field runtime, hardware TCP ACK, external CI, or reviewer-signed evidence.
...
- [ ] If `runtime:evidence` and `security:evidence` were already regenerated for the same final source revision, `npm run delivery:evidence -- --reuse-existing-companions` links the latest `artifacts/runtime/` and `artifacts/security/` manifests instead of rerunning slower companion checks.
...
- [ ] `npm run handover:index -- --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/handover-index/<timestamp>/manifest.json` plus `manifest.md` listing the latest required handover manifests, missing/review/stale areas, `Field Action Artifacts`, and `Field Rehearsal Follow-ups` with source evidence and closure plan links.
- [ ] `npm run field:closure-plan -- --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/field-closure-plan/<timestamp>/manifest.json` plus `manifest.md` listing the field commands, `Closure Command Queue`, completion blockers, redacted required field value states, open required field values, `Field Action Artifact Actions`, `Field Rehearsal Follow-up Actions`, manual evidence actions, and done-when criteria needed to close open REVIEW/STALE/MISSING areas; status remains `OPEN` until these counts are all zero.
- [ ] `FIELD_REVIEWER` and `FIELD_SITE_NAME` are set to concrete delivery-session values before running generated field commands; `npm run manual:evidence-drafts -- --base-url=http://localhost:8080 --site-name="$env:FIELD_SITE_NAME" --reviewer="$env:FIELD_REVIEWER"` creates missing reviewer-fillable drafts under `artifacts/manual/` and writes `artifacts/manual-evidence-drafts/<timestamp>/manifest.json` plus `manifest.md`; when a latest `field:risk-register` manifest exists, the draft report includes its copyable risk acceptance rows and newly created risk-acceptance drafts use those rows instead of generic template rows; operator walkthrough drafts and the draft manifest include route/API capture guidance for login, dashboard, control-board mode, events, devices, statistics, and Swagger plus a Field Acceptance Evidence Pack Checklist for reviewer metadata, operator walkthrough, level-2 escalation criteria, traffic statistics reconciliation, control-board safety ladder, and final package linkage; existing manual evidence files are preserved unless `--force` is intentionally used.
- [ ] `FIELD_BASE_URL` is set to the actual Nginx/operator entrypoint before field command generation; local review may use `http://localhost:8080`, but delivery evidence should pass `--base-url="$env:FIELD_BASE_URL"` or rely on scripts that read `FIELD_BASE_URL`.
- [ ] `npm run manual:evidence-readiness -- --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/manual-evidence-readiness/<timestamp>/manifest.json` plus `manifest.md`, showing required manual evidence target paths, template paths, validation failures, and next actions; this checklist does not replace reviewer-filled manual evidence.
- [ ] `npm run field:readiness -- --base-url=http://localhost:8080 --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/field-readiness/<timestamp>/manifest.json` plus `manifest.md` checking Docker, Nginx/API health, env readiness, control-board TCP mode, timing values, `DRY_RUN_SAFE`/`LIVE_TCP_READY`/`LIVE_TCP_REVIEW`, Swagger exposure, optional scanner availability, and `Field Value Action Groups` for Auth/Security, LiDAR ingest, control-board TCP, and Nginx delivery owners.
- [ ] `npm run field:env-closeout -- --base-url=http://localhost:8080 --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/field-env-closeout/<timestamp>/manifest.json` plus `manifest.md` with redacted owner env skeletons, current `.env` key coverage without values, an `Append Missing Env Block` for keys absent from the local `.env`, and owner closeout checklists for reviewer/session metadata, key placeholders, and strict preflight reruns.
- [ ] `npm run field:risk-register -- --base-url=http://localhost:8080 --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/field-risk-register/<timestamp>/manifest.json` plus `manifest.md`, grouping open field/security/manual risks, `Owner Risk Briefs`, `Owner Risk Next Actions`, and reviewer-copyable `Risk Acceptance Draft Rows` without replacing `artifacts/manual/field-risk-acceptance.md`.
- [ ] `npm run field:action-board -- --base-url=http://localhost:8080 --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/field-action-board/<timestamp>/manifest.json` plus `manifest.md`, grouping remaining final-status gates by owner, priority, execution phase, mapped command, evidence path, close criteria, and a phase-ordered `Execution Queue` without replacing field evidence.
- [ ] `npm run field:gate-closure-map -- --base-url=http://localhost:8080 --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/field-gate-closure-map/<timestamp>/manifest.json` plus `manifest.md`, grouping the latest action board by command and preserving `Execution Queue Linkage` so reviewers can see which gates, categories, statuses, and close criteria each field command is expected to close.
- [ ] `npm run field:owner-briefs -- --base-url=http://localhost:8080 --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/field-owner-briefs/<timestamp>/manifest.json` plus per-owner markdown brief files generated from the latest field action board, including `ownerBriefs`, `briefFileIndex`, owner phase counts, and owner-specific `Execution Queue` rows.
- [ ] `npm run field:closeout-quickstart -- --base-url="$env:FIELD_BASE_URL" --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/field-closeout-quickstart/<timestamp>/manifest.json` plus `manifest.md`, linking the latest final status, action board, gate closure map, owner briefs, field env closeout, and requirements backlog; it includes a `Safe .env Patch Block`, evidence prerequisites, runtime prerequisites, phase queue, and owner command guidance without printing secret values or replacing field evidence.
- [ ] `npm run field:requirements-backlog -- --base-url="$env:FIELD_BASE_URL" --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/field-requirements-backlog/<timestamp>/manifest.json` plus `manifest.md`, grouping `Batched unresolved field questions` by owner, priority, action type, close criteria, and source gate; it records `FIELD_REVIEWER`, `FIELD_SITE_NAME`, and `FIELD_BASE_URL` metadata requirements so field unknowns can be gathered once instead of interrupting auto-mode development.
- [ ] `npm run handover:package -- --base-url=http://localhost:8080 --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` refreshes delivery evidence, field readiness, field risk register, manual evidence drafts, manual evidence readiness, field action board, field gate closure map, field owner briefs, field closeout quickstart, field requirements backlog, completion audit, field closure plan, and handover index in order against the delivery Nginx entrypoint, then writes `artifacts/handover-package/<timestamp>/manifest.json` plus `manifest.md` with the base URL, strict gate reasons, latest manual evidence draft/readiness/risk-register/action-board/gate-closure-map/owner-brief/field-closeout-quickstart/field-requirements-backlog/final-bundle-handoff/final-gate-classification references, latest control-board safety status, and `Field Evidence Follow-ups`; strict completion uses `npm run handover:package -- --base-url=http://localhost:8080 --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME" --strict`. The `--reuse-existing-evidence` flag is reserved for `final:refresh` after its preceding steps already regenerated those evidence refs; do not use it as a standalone final-close proof.
- [ ] Handover package `Strict Failure Items` groups every strict failure by category, owner, action type, command, and close criteria so the final reviewer can route the remaining work without reverse-engineering the combined failure sentence.
...
- [ ] `npm run final:status -- --base-url=http://localhost:8080 --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/final-status/<timestamp>/manifest.json` plus `manifest.md`; final close is allowed only when this report says `READY_TO_CLOSE` and `canMarkGoalComplete=true`, otherwise close the listed `remainingGates` using the `Gate Action Summary` action types.
...
- [ ] Final status `Source Revision Freshness` shows no dirty source state, no non-`dev` evidence branch, and no stale Git commit for handover, manual readiness, action-board, gate-closure, owner-brief, final gate classification, or delivery evidence that records Git metadata.
- [ ] `npm run final:execution-plan -- --base-url=http://localhost:8080 --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/final-execution-plan/<timestamp>/manifest.json` plus `manifest.md`, translating the latest final-status `remainingGates` into an ordered execution plan for manual evidence, field runtime, rehearsals, security evidence, source revision closeout (`git status --short --branch`; `git push origin dev`), `verify:docs-text-quality`, `ci:closeout`, `ci:status`, `completion:audit`, `handover:index`, `field:closure-plan`, strict handover packaging, and final status refresh; confirm the manual evidence section runs `field:risk-register` before `manual:evidence-drafts` and `manual:evidence-readiness`, confirm `Command Gate Coverage` maps each command back to its gate count/category/status/evidence, confirm `Closure Bundles` groups the remaining gates into `Field Input And Risk Acceptance`, `Runtime And Hardware Proof`, `Security And CI Proof`, and `Final Handover Refresh`, confirm the package refresh section exposes `Field Action Artifact Actions` before the strict package rebuild, and remember this plan does not replace actual field evidence.
- [ ] `npm run final:gate-classification -- --base-url=http://localhost:8080 --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/final-gate-classification/<timestamp>/manifest.json` plus `manifest.md`, classifying final-status gates into field configuration, manual reviewer evidence, field runtime, hardware runtime, security tooling, external CI, field acceptance, and package refresh buckets, with `bucketGateCounts`, `Owner Closeout Queue`, and per-bucket owner closeout markdown files for owner handoff; this is auto-mode routing evidence and does not replace field/manual/security completion evidence.
...
- [ ] If `-SkipDeliveryEvidence` is used during field acceptance, `npm.cmd run delivery:evidence` is run separately and the skipped package step remains REVIEW/SKIPPED evidence until the delivery manifest is attached.
...
- [ ] `npm run ci:status` records `artifacts/ci-status/<timestamp>/manifest.json` for the final pushed `dev` commit, including read-only status, `CI Closeout Checklist`, approved closeout dispatch, and underlying `gh workflow run CI --ref dev` commands; final close requires status `PASS`.
- [ ] `npm run ci` passes.
