...
- [ ] If no automatic CI run exists for the final `dev` commit, `npm run ci:closeout -- --dispatch --generated-by="$env:FIELD_REVIEWER"` is used intentionally to run `gh workflow run CI --ref dev`, wait for completion, and regenerate CI status evidence.
