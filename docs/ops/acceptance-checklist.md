...
- [ ] `npm run field:action-board` creates `artifacts/field-action-board/<timestamp>/manifest.json` plus `manifest.md`, grouping remaining final-status gates by owner, priority, mapped command, evidence path, and close criteria without replacing field evidence.
- [ ] `npm run field:owner-briefs` creates `artifacts/field-owner-briefs/<timestamp>/manifest.json` plus per-owner markdown brief files generated from the latest field action board.
