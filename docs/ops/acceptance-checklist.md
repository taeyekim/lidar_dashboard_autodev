...
- [ ] `npm run field:gate-closure-map -- --base-url=http://localhost:8080 --generated-by="$env:FIELD_REVIEWER" --site-name="$env:FIELD_SITE_NAME"` creates `artifacts/field-gate-closure-map/<timestamp>/manifest.json` plus `manifest.md`, grouping the latest action board by command so reviewers can see which gates, categories, statuses, and close criteria each field command is expected to close.
