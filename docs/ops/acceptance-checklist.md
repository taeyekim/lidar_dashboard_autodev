...
- [ ] `npm run field:closure-plan` creates `artifacts/field-closure-plan/<timestamp>/manifest.json` plus `manifest.md` listing the field commands, redacted required field value states, `Field Rehearsal Follow-up Actions`, and done-when criteria needed to close open REVIEW/STALE/MISSING areas.
