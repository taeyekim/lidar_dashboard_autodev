...
이 방식에서 "에이전트 분산"은 여러 독립 AI가 무제한으로 의사결정한다는 뜻이 아니다. 같은 Codex 세션이 역할별 체크리스트와 산출물 기준을 분리해서 적용하고, 필요할 때 사용 가능한 멀티에이전트 도구로 조사/검토를 나누는 방식이다. 따라서 개발은 실제로 가능하지만, 최종 책임은 항상 메인 Codex가 가진다.

운영 시 고정할 기준:

- PM/Tech Lead가 dev 직접 push 단위의 작업 묶음을 선택한다.
- Backend, Frontend, UI/UX, LiDAR, Hardware, Infra, Security, QA, Delivery 관점이 구현 전 리스크와 완료 기준을 제시한다.
- 구현 후 각 관점은 코드, Swagger, Prisma, UI, 테스트, 문서, 증적 중 하나 이상의 형태로 검수 결과를 남긴다.
- 실패한 검증은 수정 루프로 되돌리고, 실행하지 못한 검증은 `미검증`으로 남긴다.
- 비밀값, 실제 배포, 외부 과금, 운영 DB 영향, 실제 장비 물리 제어, 파괴적 Git 작업은 오토모드에서도 중단 조건이다.
