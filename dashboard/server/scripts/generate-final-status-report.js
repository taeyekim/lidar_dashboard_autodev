...
function gateActionRunbook(gates) {
  const runbooks = {
    AUTOMATED_REFRESH_AVAILABLE:
      "Refresh generated evidence with npm.cmd run delivery:evidence; when CI status is missing or REVIEW, run npm.cmd run ci:status first, then use npm.cmd run ci:closeout -- --dispatch --generated-by=<field-reviewer> only during the approved external CI closeout window; run npm.cmd run completion:audit, npm.cmd run handover:package -- --generated-by=<field-reviewer> --site-name=<delivery-site>, then rerun npm.cmd run final:status -- --generated-by=<field-reviewer> --site-name=<delivery-site>.",
...
function buildFinalStatusReport(input = {}) {
  ...
  if (!ciStatus) {
    addGate(gates, "CI Status", "MISSING", "Latest CI status evidence manifest is missing.", "Run npm.cmd run ci:status after the final dev push. If no matching run exists, use npm.cmd run ci:closeout -- --dispatch only during the approved external CI closeout window to intentionally trigger/wait for GitHub Actions and regenerate ci:status.", null);
    ...
    addGate(
      ...
      "Run npm.cmd run ci:status first. If no matching final dev commit run exists, use npm.cmd run ci:closeout -- --dispatch only during the approved external CI closeout window, then rerun final:status.",
