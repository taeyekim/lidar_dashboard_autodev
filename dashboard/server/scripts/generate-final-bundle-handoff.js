...
function buildBundleHandoff(input = {}) {
  ...
  const ownerCloseoutFiles = (finalGateClassification?.data?.ownerCloseoutFileIndex || []).map((item) => ({
    order: item.order,
  ...
  const classificationRemainingGateCount = finalGateClassification?.data?.summary?.remainingGateCount;
  const executionPlanRemainingGateCount = finalExecutionPlan?.data?.remainingGateCount || 0;
  const remainingGateCount =
    Number.isFinite(classificationRemainingGateCount) ? classificationRemainingGateCount : executionPlanRemainingGateCount;
  ...
  return {
    ...
    remainingGateCount,
    remainingGateCountSource: Number.isFinite(classificationRemainingGateCount)
      ? "final-gate-classification"
      : "final-execution-plan",
    executionPlanRemainingGateCount,
    classificationRemainingGateCount: Number.isFinite(classificationRemainingGateCount) ? classificationRemainingGateCount : null,
    bundleCount: bundles.length,
    totalBundleGateCount: bundles.reduce((sum, bundle) => sum + (bundle.gateCount || 0), 0),
    totalOwnerCloseoutGateCount: ownerCloseoutFiles.reduce((sum, item) => sum + (item.gateCount || 0), 0),
    metadataReview,
    bundles,
    ownerCloseoutFiles,
    guardrails: [
...
function buildIndexMarkdown(manifest) {
  return [
    ...
    `- Remaining gate count source: ${manifest.remainingGateCountSource}`,
    `- Owner closeout gate count: ${manifest.totalOwnerCloseoutGateCount}`,
